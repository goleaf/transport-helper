...
<x-supply.button :href="route('supply.dashboard')">Open Supply Dashboard</x-supply.button>
<x-supply.button :href="route('supply.proposals.index')" variant="neutral" mode="outline">Review Proposals</x-supply.button>
    </nav>
</header>
...
<a class="card bg-base-100 border border-base-300 shadow-sm workflow-card" href="{{ route('supply.imports.index') }}">
    <strong>Imports</strong>
    <span>Sales history, stock, inbound orders, reservations and product rules.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm workflow-card" href="{{ route('supply.calculations.index') }}">
    <strong>Calculations</strong>
    <span>T0/T1/T2/T3, trend, raw need, rounding and formula warnings.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm workflow-card" href="{{ route('supply.proposals.index') }}">
    <strong>Order proposals</strong>
    <span>Approve, adjust with reason, reject and convert to supplier orders.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm workflow-card" href="{{ route('supply.supplier-orders.index') }}">
    <strong>Supplier orders</strong>
    <span>Export files, prepare email drafts, approve and send safely.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm workflow-card" href="{{ route('supply.emails.index') }}">
    <strong>Inbound email</strong>
    <span>Store supplier replies, match supplier/order context and queue analysis.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm workflow-card" href="{{ route('supply.ai-extractions.index') }}">
    <strong>AI extractions</strong>
    <span>Accept, reject or keep needs-review without applying business changes.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm workflow-card" href="{{ route('supply.transport.quotes.index') }}">
    <strong>Transport quotes</strong>
    <span>Compare quote options while keeping carrier selection manual.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm workflow-card" href="{{ route('supply.logistics.index') }}">
...
<a class="card bg-base-100 border border-base-300 shadow-sm" href="{{ route('supply.proposals.index') }}">
    <strong>Order quantity approval</strong>
    <span>Approve, adjust or reject with audit evidence.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm" href="{{ route('supply.supplier-orders.index') }}">
    <strong>Supplier email sending</strong>
    <span>Drafts wait for explicit approval before send.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm" href="{{ route('supply.ai-extractions.index') }}">
    <strong>AI extraction review</strong>
    <span>Accepted extraction remains separate until a later Laravel apply service.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm" href="{{ route('supply.transport.quotes.index') }}">
...
<a class="card bg-base-100 border border-base-300 shadow-sm workflow-card" href="{{ route('supply.audit-logs.index') }}">
    <strong>Audit Logs</strong>
    <span>Trace imports, approvals, email decisions, AI review and logistics changes.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm workflow-card" href="{{ route('supply.settings.index') }}">
    <strong>Settings</strong>
    <span>Control operational values without storing secrets in code.</span>
</a>
<a class="card bg-base-100 border border-base-300 shadow-sm workflow-card" href="{{ route('supply.integrations.index') }}">
